...
public:
	...
	void delrec(void);
	//Update a record
	void update(char* record);
	//Turn off a record's update lock
	void updateoff(void);
private:
	...
	void readFH(void);
public:
	//Indicates the key type for exterior elements (e.g. main program)
	char getKeyType(void);
	//Indicates open status: if a file is opened and in which mode
	void isOpen(bool* status);
	//Sends information such as file size, key type and hash function
	string* getInfo(void);
	//Indicates the read mode value
	int getOpenMode(void);
	//Fills an array with the number of records per block
	void getBlockDesc(int*);
	//Indicates the content of a block's Data
	int RecordsInBlock(char*, int);
	//Returns the file size
	int getFileSize(void);
	//Returns the percentage of place available for records
	double emptyVolume(void);
};